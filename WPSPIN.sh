...
sudo airmon-ng | sed '1,3d' | sed '$d' > /tmp/airmon.txt  # with sed and airmon-ng we take out the interesting information of airmon-ng command
...
do 
 count=1                                                                      # we start from one             
  if [ "$i" == "MON_INTERFACE" ]; then                                        # we start with the array for the mode monitor capable interfaces
      ...
      done < <( cat /tmp/airmon.txt | awk -F' ' '{ print $2 }')               # we take the first field that is wlanX or monX in airmon-ng display 
...
MON_ATTACK=$(sudo airmon-ng check kill &> /dev/null && sudo airmon-ng start $(echo "${MON_INTERFACE[${i}]}") | grep enabled |  awk -F' ' '{ print $9 }' |  sed -e 's/^.\{6\}//' | sed 's/.$//' )  # we activate mode monitor
  #sudo ifconfig $(echo "${MON_INTERFACE[${i}]}") down
fi
...
if [ "$AIRMON_INTERFACE" == 1 ] && [ "$IW_INTERFACE" == 1 ]    ; then         # if there is just one interface and no mode monitor interface, this single interface
 sudo ifconfig $(echo "${MON_INTERFACE[${i}]}") down
 MON_ATTACK=$(sudo airmon-ng check kill &> /dev/null && sudo airmon-ng start $(echo "${MON_INTERFACE[${i}]}") | grep enabled |  awk -F' ' '{ print $9 }' |  sed -e 's/^.\{6\}//' | sed 's/.$//' )  # we activate mode monitor automatically
...
if [ "$MON_ATTACK" == "" ] && [ "$IW_MONX" == 0 ]; then                        # If there is no interface in monitor mode detected      
  ...
  MON_ATTACK=$(sudo airmon-ng check kill &> /dev/null && sudo airmon-ng start $(echo "${MON_INTERFACE[${i}]}") | grep enabled |  awk -F' ' '{ print $9 }' |  sed -e 's/^.\{6\}//' | sed 's/.$//' )   # We start modemonitor                    
...
MONITORIZED=$( echo "$CHOIX" | grep mon )                                   # in case the interface is in mode monitor we create monotorized
  ...
  if [ "$MONITORIZED"  == "" ]; then                                        # if monotorized is empty it means the ethX or wlanX has to be put into monitor mode
    ...
    MON_ATTACK=$( sudo airmon-ng check kill &> /dev/null && sudo airmon-ng start $CHOIX | grep enabled |  awk -F' ' '{ print $9 }' |  sed -e 's/^.\{6\}//' | sed 's/.$//' ) # we activate mode monitor an in the meanwhile we grap the 
     RT_CHECK=$( echo ${MON_CHIPSET[${i}]} | grep RalinkRT2870 )                  # filter for rt3070 that associate better if wlan is up
     if [ -n "${RT_CHECK}" ]; then
       sudo ifconfig $(echo ${MON_INTERFACE[${i}]}) up
     else
...
airmon-ng stop $MON_ATTACK &> /dev/null
service network-manager start &> /dev/null                                                                       # option exit
     CLEAN
     CIAO
     exit 0
